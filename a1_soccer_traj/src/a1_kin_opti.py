...
class A1KinematicsOpti:
	def __init__(self):
		...
		self.ef = 'FR'
		# state
		self.input_num = 6 # velocity on base
		self.base_pose_num = 6
		self.FR_state_num = 3
		self.FR_vel_num = 3
		self.FL_state_num = 3
		self.FL_vel_num = 3
	...
	def solve(self):
		...
		self.opti.subject_to(self.base_poses[:, 0] == self.init_base_pose)
		self.opti.subject_to(self.get_end_effector_pos(0)[:3] == self.init_ef_pos[:3])
		...
		for i in range(self.total_grid):
			...
			self.opti.subject_to(
				ca.vertcat(base_k[0], base_k[1]) == ca.mtimes(triangle_mat, self.lambdas[:, i]))
			...
			if i is not self.total_grid-1:
				...
				base_p = base_k + (base_vel_k + last_base_vel) * deltaT / 2.0
		...
		self.__addEFNodeCost(self.final_ef_pos, -1, self.w_ef_final)
